...
const Listmodal: React.FC<Props> = ({ quests, categories }) => {
		...
		const response = await fetch("/api/list", {
			...
			body: JSON.stringify({
				...
				tasks: values.quests.map((e: string) => {
					return e.substring(e.length - (e.length - e.indexOf("&$$&")) + 4, e.length);
				}),
			}),
		});
		if (!response.ok) {
	...
	return (
		<div className="bg-offwhite-200 border-offwhite-50  border-solid border-2   rounded-lg py-2 px-2 text-offwhite-50 ">
			...
			<Form form={form} layout="vertical" style={{ width: "100%" }} name="control-hooks" onFinish={onFinish}>
				...
				<Form.Item
					...
					rules={[
						...
						{
							max: 120,
							message: "List descriptions names have a maximum of 120 characters",
						},
					]}
				>
